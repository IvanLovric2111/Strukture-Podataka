...
Position Trazi(char* surname, Position head);
...
int main(int argc, char** argv)
{
	...
	UnosP(&head, "Ivan", "Ivanovic", 20);
	UnosK(&head, "Ante", "Antic", 19);
	UnosIza(head->next, &head, "Mara", "Martic", 28);
	UnosIspred(head->next, &head, "Lucija", "Lukic", 24);
	Sortiraj(&head);
	Ispis(head->next);
	el = Trazi("Antic", &head);
	Brisi(el, &head);
